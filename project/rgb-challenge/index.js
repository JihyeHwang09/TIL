...
// 이벤트 리스너 등록해주기
// option클래스를 가지고 있는 요소들(총 3개)에 접근해서 순회하면서 
...
document.querySelectorAll(".option").forEach((optionEl, index) => {
    optionEl.addEventListener("click", e => {
        ...
        if (answer === index) {
            // if(options[index] === options[answer])로 비교할 수도 있으나 
            // 둘 다 숫자니까 (answer ===index)로 비교해도 된다.
            score++;
            // 정답인 보기를 클릭했을 때, THAT'S RIGHT!이라는 메시지와 NEXT STAGE버튼을 보여주자.
            // rightModal 요소를 가져와서 open클래스를 추가해주자.
            rightModalEl.classList.add('open')
            // 정답이 아닌 보기를 클릭했을 때
        } else {
            // score를 0으로 초기화하기 전에(score값이 날아가기 전에) 
            // score를 화면에 띄워줘야 하기 때문에 'score=0' 코드보다 먼저 적어준다.
            // score-in-modal 요소를 가져와서 score에 쌓인 점수를 html에 넣어준다. ('SCORE:' 옆에 띄울 점수)
            ...
            // wrongModal요소를 가져와서 open클래스를 붙여주자.
        }
        document.querySelector('.score-text').textContent = score;
        // newStage()호출할 때가 아니라, next stage나 play again버튼을 눌렀을 때
        //  색깔을 변경해줘야 하므로 newStage()코드를 여기에 써주지 않는다.
    });
});
// 매 판마다 다시 만들어줘야 하므로 -> 재사용해야 하니까 함수로 만들어줘야겠다.
function newStage() {
    // 색깔 바꿔주기
    ...
    // document.querySelectorAll('.option')는 nodeList라서 forEach메소드를 사용할 수 있다.
    // 첫 번째, 두 번째, 세 번째 옵션이 들어가면서 각각 다른 색깔을 넣어준다.
    // nodeList의 forEach 메소드에도 currentValue, currentIndex, listObj를 파라미터로 받는다. -> 배열처럼 index를 사용할 수 있다.
    document.querySelectorAll(".option").forEach((optionEl, index) => {
    ...
    // 색깔 코드를 글씨로 표현해주는 color-text 클래스에 3가지 보기 중 하나의  rgb코드를 글씨로 넣어준다.
...
document.querySelector('.play-again').addEventListener('click', e => {
    ...
    // document.querySelector('.wrong-modal')이 여러 번 쓰이니까 변수에 담아서 사용하자.
    // wrongModal요소에 open 클래스를 떼준다. 
})